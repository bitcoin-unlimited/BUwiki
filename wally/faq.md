...
## What actions can be selected on the top of Wally's home page?
...
**Important note!**  For any account that was set up with a PIN, you will need that account's PIN to access the account's funds. So please, remember the PINS forever and also write them down and save them somewhere safe. Secondly, after setting up each account, be sure to go to Wally's settings page to see and write down the Account Recovery Secret Phrase for each account.  (See SETTINGS below.) 
...
Yes. Under Account Balances, just tap on an account name and a Account History page will be displayed, showing "sends" with an orange arrow and receipts with a green arrow. Tap on a transaction and the details for that transaction will be displayed. Expect a short delay as Wally gets the history information from the blockchain.
